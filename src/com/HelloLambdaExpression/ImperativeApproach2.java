/**
 * Program to find the old way of calculationg the total with discount 10% above Rs.20
 */
package com.HelloLambdaExpression;

/**
 * @author ishwaraju
 */
import java.math.BigDecimal;
import java.util.Arrays;
import java.util.List;

public class ImperativeApproach2 {

	private static BigDecimal calculate(List<BigDecimal> prices) {
		BigDecimal totalDiscount = BigDecimal.ZERO;
		for (BigDecimal price : prices)
			if (price.compareTo(BigDecimal.valueOf(20)) > 0)
				totalDiscount = totalDiscount.add(price.multiply(BigDecimal.valueOf(0.9)));
		return totalDiscount;
	}

	public static void main(String[] args) {
		final List<BigDecimal> prices = Arrays.asList(new BigDecimal("10"), new BigDecimal("30"), new BigDecimal("17"),
				new BigDecimal("20"), new BigDecimal("15"), new BigDecimal("18"), new BigDecimal("45"),
				new BigDecimal("12"));
		BigDecimal total = calculate(prices);
		System.out.println("Total amount := " + total);
	}

}
